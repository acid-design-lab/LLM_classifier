...
from gpt.gpt_funcs import request_completion, create_request_template
...
from samplers.occurrence_sampler import OccurrenceSampler
from samplers.strict_sampler import StrictSampler
from mistral.mistral_funcs import create_request_template_mistral
...
sample_features_train, sample_features_test, sample_targets_train, sample_targets_test = load_dataset(
    link=config["dataset"],
...
for request in range(10): #len(sample_features_test)
    ...
    if config["nnet"] == "gpt":
        template = create_request_template(samples,
                                           seed=config["seed"],
        ...
        try:
            ...
            # raise
            log.append(f"Request_{request} missed\n"
            ...
            history.append(f"request: {sample_features_test[request]}\n"
                           f"response: {completion_response.choices[0].message.content}\n"
                           f"true: {sample_targets_test[request]}")

    elif config["nnet"] == "mistral":
        template = create_request_template_mistral(samples,
                                                   seed=config["seed"])
        try:
            completion_response = request_completion(question=sample_features_test[request],
                                                     name=f"{config['subject']}_doctor",
...
#     calculate_classification_metrics(y_true=string_to_continuous(income_data=sample_targets_test,
#                                                                  classes=config["classes"]),
#                                      y_pred=string_to_continuous(income_data=result,
#                                                                  classes=config["classes"]),
#                                      config=config,
#                                      path=path,
#                                      title=title
