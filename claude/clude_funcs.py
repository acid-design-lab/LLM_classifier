...
def conversation(client, prompt, new_conversation=True):
    ...
    if new_conversation:
        prompt = "I'm planning a trip to Paris next spring. Any recommendations on where I should visit or what I " \
                 "should be sure to do?"
        response = client.post("https://api.anthropic.com/v2/conversations", json={"messages": [{"content": prompt}]})
        return response.json()
    else:
        followup = "I'm a big art lover - what museum would you recommend as a must-see?"
        response = client.post("https://api.anthropic.com/v2/conversations",
...
def completions(client, prompt):
    ...
    response = client.post("https://api.anthropic.com/v2/completions", json={"prompt": prompt})
    return response.json()
...
def answer(client, quastion):
    ...
    response = client.post("https://api.anthropic.com/v2/answers", json={"question": question})
    return response.json()
...
def classifications(client, content):
    ...
    response = client.post("https://api.anthropic.com/v2/classifications",
...
def embedding(client, text):
    ...
    response = client.post("https://api.anthropic.com/v2/embeddings", json={'content': text})
    return response.json()
